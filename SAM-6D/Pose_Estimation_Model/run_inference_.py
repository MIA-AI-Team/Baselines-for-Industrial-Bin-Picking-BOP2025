...
def init(det_score_thresh = 0.37):
    ...
    # args = get_parser()
    exp_name = model + '_' + \
        osp.splitext(args.config.split("/")[-1])[0] + '_id' + str(exp_id)
...
def get_test_data(camera, detections, ply_obj_path, det_score_thresh, cfg):
    dets = []
    dets_ = detections
    for det in dets_:
...
if __name__ == "__main__":
    ...
    template_dir = "/home/yangchen/SAM-6D/Pose_Estimation_Model/template"
    all_tem, all_tem_pts, all_tem_choose = get_templates(template_dir, cfg)
    with torch.no_grad():
